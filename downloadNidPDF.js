// Function to generate and download NID card as PDF
function downloadNidPDF() {
    // Get all form values
    const formData = {
...
<body>
    ...
    setTimeout(() => {
        ...
        html2canvas(printWindow.document.getElementById('pdfcontainer'), {
            scale: 5,
            useCORS: true,
            logging: false
        }).then(canvas => {
            ...
            // Download the PDF
            pdf.save(`NID_${formData.nidNumber}.pdf`);
...
document.getElementById('downloadBtn').addEventListener('click', function() {
    ...
    if (isValid) {
        ...
        this.innerHTML = '<span class="spinner"></span> ' + (document.activeElement.id === 'englishBtn' ? 'Generating PDF...' : 'PDF তৈরি হচ্ছে...');
        this.disabled = true;
